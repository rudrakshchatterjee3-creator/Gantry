...
There is no backend database. Every piece of dashboard state — incident resolutions, zone overrides, ambient telemetry — lives in Zustand stores in the browser. This is a real, disclosed constraint (a page refresh loses session state), not an oversight.
